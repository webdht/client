...
const get_active = `${registration.scope}clients-active`;
onfetch = e => e.respondWith((async () => {
	...
	if (e.request.url == get_id) {
		...
		let active_ids = await clients.matchAll({ includeUncontrolled: true, type: 'window' });
		active_ids = active_ids.map(wc => wc.id);
